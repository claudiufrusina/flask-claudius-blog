...
@app.route("/")
def homepage():
    # riceviamo page number via url e lo castiamo a intero partendo da id 1
    page_number = request.args.get('page', 1, type=int)
    # data una determinata pagina estraggo solamente i primi 6 elementi 
    # e qualora fuori range mostro errore 404
    posts = Post.query.order_by(Post.created_at.desc()).paginate(page=page_number, 
        per_page=app.config["TICKERS_PER_PAGE"], error_out=True)
    if posts.has_next:
    ...
    if form.validate_on_submit():
        ...
        if form.image.data:
            try:
                ...
                db.session.commit()
                flash("C'è stato un problema durate l'upload. Cambia immagine e riprova")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            flash('Username e password non validi')
